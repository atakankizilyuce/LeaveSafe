...
// playWAV writes WAV data to a temp file and plays it with paplay or aplay.
...
func playWAV(wavData []byte, stopCh <-chan struct{}) error {
	...
	if _, err := exec.LookPath("paplay"); err == nil {
		cmd = exec.Command("paplay", tempFile)
	} else {
		cmd = exec.Command("aplay", tempFile)
	}
	playerCmd = cmd
	playerMu.Unlock()
...
// stopPlayback kills any running audio process and cleans up.
func stopPlayback() {
	playerMu.Lock()
